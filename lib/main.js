...
async function build(target = "") {
  ...
  if (projects.length > 1) {
    ...
    if (projects[0]) {
      ...
      atom.notifications.addError(
        "There is no projects to build in the current workspace"
      );
      return;
    }
  }
  ...
  if (!has_makefile(project)) {
    atom.notifications.addError("Makefile not found");
    return;
  }
  ...
  if (!has_command(make)) {
    atom.notifications.addError(
      ...
      "please make sure that make is installed and it's in your path"
    );
    return;
  }
  ...
  ps.on("close", (code) => {
    ...
    if (code) {
      atom.notifications.addError("Failed to build project");
    } else {
      atom.notifications.addSuccess("Project has been built successfully");
    }
  });
}
